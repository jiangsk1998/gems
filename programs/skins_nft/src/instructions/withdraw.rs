...
// System Program 的 transfer 比较特殊，它既检查签名又检查 owner，所以 PDA 过不了。其他大部分程序（Token、Metaplex 等）只检查签名，所以 new_with_signer 可以正常用。
    ...
    //     amount,
    // )?;
    ctx.accounts.treasury.to_account_info().try_borrow_mut_lamports()?.checked_sub(amount).ok_or(SkinsNftError::InsufficientFunds)?;
    ctx.accounts.recipient.to_account_info().try_borrow_mut_lamports()?.checked_add(amount).ok_or(SkinsNftError::MathOverflow)?;    
    msg!(
