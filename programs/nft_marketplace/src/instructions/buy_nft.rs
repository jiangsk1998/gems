...
pub struct BuyNft<'info> {
    ...
    pub buyer_game_account: Account<'info, TokenAccount>,
    ...
    pub seller_game_account: Account<'info, TokenAccount>,
...
pub fn buy_nft(ctx: Context<BuyNft>) -> Result<()> {
    ...
    require!(
        ctx.accounts.buyer_game_account.amount >= price,
    ...
    let game_transfer_ctx = CpiContext::new(
        ...
        Transfer {
            from: ctx.accounts.buyer_game_account.to_account_info(), // 买家 $Tangaga ATA
            to: ctx.accounts.seller_game_account.to_account_info(),  // 卖家 $Tangaga ATA
            authority: ctx.accounts.buyer.to_account_info(),         // 买家签名
        },
    );
    token::transfer(game_transfer_ctx, price)?; // 转账 price 数量的 $Tangaga
